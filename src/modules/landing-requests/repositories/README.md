...
- `landing-request.repository.interface.ts` — assinatura `ILandingRequestRepository`.
- `landing-request.repository.ts` — implementação `LandingRequestRepository implements ILandingRequestRepository`.
...
- Registrar o repository como provider no `landing-requests.module.ts`.
