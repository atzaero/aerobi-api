...
- `pilot-landing.repository.interface.ts` — assinatura `IPilotLandingRepository`.
- `pilot-landing.repository.ts` — implementação `PilotLandingRepository implements IPilotLandingRepository`.
...
- Registrar o repository como provider no `pilot-landings.module.ts`.
