...
- `create-aerodrome-feedback.dto.ts` — body de `POST` (também usado como input do service).
- `update-aerodrome-feedback.dto.ts` — body de `PATCH`.
- `list-aerodrome-feedbacks-query.dto.ts` — query de `GET`; extende `BasePaginationQueryDTO`.
- `aerodrome-feedback-response.dto.ts` — resposta de item único.
- `aerodrome-feedbacks-paginated-response.dto.ts` — resposta de listagem; extende `BasePaginatedResponseDTO`.
