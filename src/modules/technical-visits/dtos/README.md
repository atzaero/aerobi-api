...
- `create-technical-visit.dto.ts` — body de `POST` (também usado como input do service).
- `update-technical-visit.dto.ts` — body de `PATCH`.
- `list-technical-visits-query.dto.ts` — query de `GET`; extende `BasePaginationQueryDTO`.
- `technical-visit-response.dto.ts` — resposta de item único.
- `technical-visits-paginated-response.dto.ts` — resposta de listagem; extende `BasePaginatedResponseDTO`.
