...
import { ListAircraftReadingsService } from './list-aircraft-readings.service';

describe('ListAircraftReadingsService', () => {
  let service: ListAircraftReadingsService;
  let findMany: jest.Mock;
  let count: jest.Mock;
  let getPresignedUrl: jest.Mock;

  const entity = (over: Partial<AircraftReading> = {}): AircraftReading => ({
    id: 'r-1',
    registration: 'PR-ZTT',
    confidence: '0.98',
  ...
  beforeEach(() => {
    ...
    const repo = { findMany, count } as unknown as AircraftReadingRepository;
    const storage = { getPresignedUrl } as unknown as StorageService;
    service = new ListAircraftReadingsService(repo, storage);
  });
