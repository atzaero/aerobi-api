...
- `create-aerodrome-group.dto.ts` — body de `POST` (também usado como input do service).
- `update-aerodrome-group.dto.ts` — body de `PATCH`.
- `list-aerodrome-groups-query.dto.ts` — query de `GET`; extende `BasePaginationQueryDTO`.
- `aerodrome-group-response.dto.ts` — resposta de item único.
- `aerodrome-groups-paginated-response.dto.ts` — resposta de listagem; extende `BasePaginatedResponseDTO`.
