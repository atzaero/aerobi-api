...
- `create-geojson.dto.ts` — body de `POST` (também usado como input do service).
- `update-geojson.dto.ts` — body de `PATCH`.
- `list-geojsons-query.dto.ts` — query de `GET`; extende `BasePaginationQueryDTO`.
- `geojson-response.dto.ts` — resposta de item único.
- `geojsons-paginated-response.dto.ts` — resposta de listagem; extende `BasePaginatedResponseDTO`.
