...
import { ExportGroupsService } from './export-groups.service';
...
describe('ExportGroupsService', () => {
  let service: ExportGroupsService;
  ...
  beforeEach(() => {
    ...
    service = new ExportGroupsService(
      repo,
      userRepo,
  ...
  it('ADMIN: where vazio, busca MAX+1 sem paginação, cabeçalho + linha', async () => {
    findMany.mockResolvedValue([
      buildGroupFixture({ name: 'Interior', uf: Uf.SP }),
    ]);
