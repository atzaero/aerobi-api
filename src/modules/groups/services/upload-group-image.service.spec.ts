...
import { UploadGroupImageService } from './upload-group-image.service';
...
describe('UploadGroupImageService', () => {
  let service: UploadGroupImageService;
  ...
  beforeEach(() => {
    ...
    service = new UploadGroupImageService(
      groupRepo,
      imageRepo,
      storage,
  ...
  it('aceita webp válido (magic bytes RIFF/WEBP)', async () => {
    ...
    createActiveImage.mockResolvedValue(
      buildGroupFixture({ id, imageKey: 'groups/x/images/y.webp' }),
    );
