...
import { groupExportColumns } from '../mappers/group-export.columns';
 ...
 */
export interface ExportGroupsResult {
  csv: string;
  truncated: boolean;
  total: number;
}
...
@Injectable()
export class ExportGroupsService {
  private readonly logger = new Logger(ExportGroupsService.name);
  ...
  async execute(
    query: ExportGroupsQueryDTO,
    actor: AuthenticatedUser,
  ): Promise<ExportGroupsResult> {
    /**
    ...
    if (rows.length > EXPORT_MAX_ROWS) {
      ...
      return {
        csv: toCsv(rows.slice(0, EXPORT_MAX_ROWS), groupExportColumns),
        truncated: true,
        total,
      };
    }
    ...
    return {
      csv: toCsv(rows, groupExportColumns),
      truncated: false,
      total: rows.length,
    };
  }
