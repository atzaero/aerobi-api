...
- `create-aerodrome.dto.ts` — body de `POST` (também usado como input do service).
- `update-aerodrome.dto.ts` — body de `PATCH`.
- `list-aerodromes-query.dto.ts` — query de `GET`; extende `BasePaginationQueryDTO`.
- `aerodrome-response.dto.ts` — resposta de item único.
- `aerodromes-paginated-response.dto.ts` — resposta de listagem; extende `BasePaginatedResponseDTO`.
