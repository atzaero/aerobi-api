...
- **Comandos tipo slash**: [.claude/commands/](.claude/commands/) são a referência (`/branch`, `/commit`, `/review`, `/pr`, `/merge`, `/babysit-pr` (acompanha PR/CI até verde), `/complete-flow`, `/scaffold-module`, …). No Cursor, [.cursor/commands/](.cursor/commands/) usa os mesmos nomes mas **só aponta** para os ficheiros em `.claude/commands/` (sem duplicar o texto) — seguir o workflow de cada um. Skill `babysit-pr` em [.claude/skills/babysit-pr/](.claude/skills/babysit-pr/).
...
- **Swagger/OpenAPI**: ao criar/alterar módulo, seguir [`src/bootstrap/swagger/README.md`](src/bootstrap/swagger/README.md) (fonte canônica) — registrar a tag no array `TAGS` de [`src/bootstrap/swagger/setup-swagger.ts`](src/bootstrap/swagger/setup-swagger.ts) no **bloco por dependência** (Identidade → Estrutura → Operações → Integrações → Sistema), nome idêntico em `@ApiTags`/`TAGS`/scaffold, decoradores em `docs/*.docs.ts` com segurança casando o guard real da rota.
