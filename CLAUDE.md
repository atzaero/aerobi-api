...
- **Comandos tipo slash**: [.claude/commands/](.claude/commands/) são a referência (`/branch`, `/commit`, `/review`, `/pr`, `/merge`, `/complete-flow`, `/scaffold-module`, …). No Cursor há espelhos em [.cursor/commands/](.cursor/commands/) com os mesmos nomes quando existirem — seguir o workflow de cada um.
